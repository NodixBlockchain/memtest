...
OS_API_C_FUNC(void) mark_zone(mem_zone *zone, unsigned int scan_id)
{
	...
	if (get_area(zone->area_id) == PTR_NULL)return;
...
OS_API_C_FUNC(unsigned int) do_mark_sweep(unsigned int area_id, unsigned int delay)
{
	...
	mark_modz_zones			(area_ptr->zones_buffer,mem_add(area_ptr->zones_buffer,MAX_MEM_ZONES*sizeof(mem_zone)));
...
OS_API_C_FUNC(unsigned int) get_zone_area_type(mem_zone_ref_ptr zone)
{
